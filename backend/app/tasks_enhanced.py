...
def create_chart_ready_data(db: Session, user) -> Dict:
    ...
    try:
        ...
        for record in matched_records:
            pharmacy_name = record['invoice'].pharmacy_name
            if pharmacy_name not in pharmacy_revenue:
                pharmacy_revenue[pharmacy_name] = 0.0
            pharmacy_revenue[pharmacy_name] += _calc_revenue(record)
        
        # Group by doctor
        doctor_revenue = {}
        for record in matched_records:
            doctor_name = record['master'].doctor_names
            if doctor_name not in doctor_revenue:
                doctor_revenue[doctor_name] = 0.0
            doctor_revenue[doctor_name] += _calc_revenue(record)
        
        # Group by rep
        rep_revenue = {}
        for record in matched_records:
            rep_name = record['master'].rep_names
            if rep_name not in rep_revenue:
                rep_revenue[rep_name] = 0.0
            rep_revenue[rep_name] += _calc_revenue(record)
        ...
        pharmacy_chart_data = [
            ...
            for name, revenue in sorted(pharmacy_revenue.items(), key=lambda x: x[1], reverse=True)
        ]
        
        doctor_chart_data = [
            {"doctor_name": name, "revenue": float(revenue)}
            for name, revenue in sorted(doctor_revenue.items(), key=lambda x: x[1], reverse=True)
        ]
        
        rep_chart_data = [
            {"rep_name": name, "revenue": float(revenue)}
            for name, revenue in sorted(rep_revenue.items(), key=lambda x: x[1], reverse=True)
        ]
